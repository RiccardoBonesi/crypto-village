...
async function BookRestaurant(transactionRequest) {
    ...
    // creo la prenotazione (new oggetto asset)
    // aggiungo questa prenotazione nell'array di DailyReservation
    // restaurant contiene un array di dailyReservation
    ...
    if(!dayExist){
        ...
        var day = await factory.newResource('org.digitalpayment', 'DailyReservations');
        day.date = date;
        day.restaurant = restaurant;
        day.availableSeats = restaurant.totalSeats - seats;
    }

/*
    console.log(restaurant.restaurantId);
    if (dailyReservationsRegistry.legth != 0) {
        for (i = 0; i < dailyReservationsRegistry.legth; i++) {
            console.log(dailyReservationsRegistry[i].restaurant.restaurantId);
            // check if an instance with this restaurantId and date already exist
            if (dailyReservationsRegistry[i].restaurant.restaurantId === restaurant.restaurantId &&
                dailyReservationsRegistry[i].date === date) {

                dailyReservationsRegistry[i].reservations.push(reservation);

                // check if there are enough seats available
                if (dailyReservationsRegistry[i].availableSeats >= seats) {
                    dailyReservationsRegistry[i].availableSeats -= seats;
                    dayExist = true;
                }
                else {
                    throw new Error('Sold out.');
