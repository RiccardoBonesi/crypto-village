/**
* Wallet transaction
* @param {org.cryptovillage.WalletTransfer} transactionRequest
* @transaction
*/
async function walletTransfer(transactionRequest) {    

    var from = transactionRequest.from;
    var to = transactionRequest.to;
    ...
    if (!fromCustomerFamily || !fromCustomerFamily.includes(to.owner)) {
        if (from.balance < amount + 5) {
            throw new Error("Insufficient funds");
        }

        from.balance -= amount + 5;
    ...
    let assetRegistry = await getAssetRegistry('org.cryptovillage.Wallet');
    await assetRegistry.update(from);
    await assetRegistry.update(to);
}

/**
* Wallet top-up
* @param {org.cryptovillage.TopUpWallet} topUpWallet
* @transaction
*/
...
/**
* Payment
* @param {org.cryptovillage.Payment} payment
* @transaction
*/
async function payment(payment) {
    ...
    let paymentNotification = getFactory().newEvent('org.cryptovillage', 'PaymentNotification');
...
/**
 * BuyLotteryTicket
 * @param {org.cryptovillage.BuyLotteryTicket} transactionRequest
 * @transaction
 */
async function BuyLotteryTicket(transactionRequest) {
    ...
    let ticketRegistry = await getAssetRegistry('org.cryptovillage.LotteryTicket');
    let factory = await getFactory();    
    let lottery = transactionRequest.lottery;
    let ticketOwner = transactionRequest.ticketOwner;
    
    let ticketId = lottery.lotteryId + ticketOwner.walletId;
    let ticket = await factory.newResource('org.cryptovillage', 'LotteryTicket', ticketId);
    ...
    let walletRegistry = await getAssetRegistry('org.cryptovillage.Wallet');
    ...
    let lotteryRegistry = await getAssetRegistry('org.cryptovillage.Lottery');
    await lotteryRegistry.update(lottery);
}

/**
 * DrawLottery
 * @param {org.cryptovillage.DrawLottery} transactionRequest
 * @transaction
 */
...
/**
 * BookEvent
 * @param {org.cryptovillage.BookEvent} transactionRequest
 * @transaction
 */
async function BookEvent(transactionRequest) {
    ...
    let eventRegistry = await getAssetRegistry('org.cryptovillage.RealEvent');
 ...
 * @param {org.cryptovillage.DeleteEventReservation} transactionRequest
 * @transaction
 */
...
/**
 * BookBeachUmbrella
 * @param {org.cryptovillage.BookBeachUmbrella} transactionRequest
 * @transaction
 */
async function BookBeachUmbrella(transactionRequest) {
    ...
    let reservationsRegistry = await getAssetRegistry('org.cryptovillage.BeachUmbrellaReservation');
    let factory = await getFactory();
    let reservation = await factory.newResource('org.cryptovillage', 'BeachUmbrellaReservation', reservationDetails.reservationId);
    ...
    let beachUmbrellaRegistry = await getAssetRegistry('org.cryptovillage.BeachUmbrella');
    await beachUmbrellaRegistry.update(beachUmbrella);
}

/**
 * DeleteUmbrellaReservation
 * @param {org.cryptovillage.DeleteUmbrellaReservation} transactionRequest
 * @transaction
 */
...
/**
 * BookRestaurant
 * @param {org.cryptovillage.BookRestaurant} transactionRequest
 * @transaction
 */
async function BookRestaurant(transactionRequest) {
    ...
    // buld new reservation
    let reservationsRegistry = await getAssetRegistry('org.cryptovillage.RestaurantReservation');
    let factory = await getFactory();
    var reservation = await factory.newResource('org.cryptovillage', 'RestaurantReservation', reservationDetails.reservationId);
    var dailyReservationsRegistry = await getAssetRegistry('org.cryptovillage.DailyReservations');
    ...
    if (!dayExist) {
        ...
        let day = await factory.newResource('org.cryptovillage', 'DailyReservations', day_id);
        day.date = date;
    ...
    let restaurantRegistry = await getAssetRegistry('org.cryptovillage.Restaurant');
 ...
 * @param {org.cryptovillage.DeleteRestaurantReservation} transactionRequest
 * @transaction
 */
