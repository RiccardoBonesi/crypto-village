...
async function accountTransfer(accountTransfer) {
    ...
    if (!fromCustomerFamily || !fromCustomerFamily.includes(to)) {
        console.log("Array non definito o non in famiglia");
        fee = true;
    }

    if (fee){
        accountTransfer.from.balance -= (amount + 5);
    } else {
        accountTransfer.from.balance -= amount;
    }

    // perform transaction    
    accountTransfer.to.balance += amount;
